...
<nav class="fixed top-0 left-0 w-full bg-[#E9B9C5] shadow-md z-[1000]">
  <div class="flex items-center justify-between px-4 sm:px-8 h-16">
    ...
    <div class="hidden md:flex flex-1 justify-center">
      <div class="flex items-center space-x-12 text-white font-medium">
        ...
        <a href="informasiortu" class="px-4 py-2 rounded-md hover:bg-[#B9E9DD] hover:text-[#53AFA2] transition">
          Informasi Kesehatan
        </a>
      </div>
    </div>
    ...
    <div class="flex items-center space-x-3">
      ...
      <!-- Tombol Login (mobile & desktop) -->
      <a href="{{ route('login') }}"
        class="px-5 py-2 bg-white text-[#E9B9C5] font-semibold rounded-lg hover:bg-[#B9E9DD] hover:text-[#53AFA2] transition">
        Login
      </a>
    </div>
  </div>
  ...
  <div id="sidebar"
    ...
    <nav class="flex flex-col space-y-4 mt-4 px-6">
      ...
      <!-- Tambahkan Login juga di sidebar -->
      <a href="{{ route('login') }}" class="text-white font-medium border-b border-white pb-1 hover:text-[#B9E9DD] hover:border-[#B9E9DD] transition duration-300">Login</a>
    </nav>
  </div>
</nav>
